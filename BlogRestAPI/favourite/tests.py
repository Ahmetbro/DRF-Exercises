...
class FavouriteCreateList(APITestCase):
    url = reverse("favourite:favs")
    url_login = reverse("token_obtain_pair")

    def setUp(self):
        self.username = "ahmett"
...
class FavouriteUpdateDelete(APITestCase):
    ...
    def setUp(self):
        ...
        self.password = "test1234"
        self.user = User.objects.create_user(username=self.username, password=self.password)
        self.user2 = User.objects.create_user(username="ahmet2", password=self.password)
        self.post = Post.objects.create(title="Başlık", content="İçerik")
        self.favourite = Favourite.objects.create(content="deneme", post=self.post, user=self.user)
        self.url = reverse("favourite:fav-detail", kwargs={"pk": self.favourite.pk})
        self.test_jwt_authentication()

    def test_jwt_authentication(self, username="ahmett", password="test1234"):
    ...
    def test_fav_delete_different_user(self):
        self.test_jwt_authentication("ahmet2")
        response = self.client.delete(self.url)
