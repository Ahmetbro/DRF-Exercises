...
class CommentUpdateDeleteTest(APITestCase):
    ...
    def setUp(self):
        ...
        self.post = Post.objects.create(title="deneme", content="içerik")
        self.user = User.objects.create_user(username=self.username, password=self.password)
        self.user2 = User.objects.create_user(username="ahmet12345", password=self.password)
        self.comment = Comment.objects.create(content="içerik yorum", user=self.user, post=self.post)
        self.url = reverse("comment:post-detail", kwargs={'pk': self.comment.pk})
        self.test_jwt_authentication()
    ...
    def test_delete_other_user(self):
        self.test_jwt_authentication("ahmet12345")
        response = self.client.delete(self.url)
